...
#
# PublishTrimmed is deliberately off. Microsoft.Data.Sqlite uses reflection, and the saving is
# not worth a binary that fails to open its own database on a machine nobody is watching. If it
# is ever turned on, the published binary has to be run and checked against a real database, a
# real policy and a real event log before the change is kept.
